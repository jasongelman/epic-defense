...
export class HyruleTower extends Tower {
    constructor(x, y) {
        super(x, y);
        ...
        this.frameTimer = 0;
        this.frameInterval = 0.05; // Fast animation (Spin attack?)
    }
    ...
    update(dt, enemies) {
        ...
        if (event && event.type === 'shoot') {
            // Melee "projectile" (instant hit visual) OR short range slash
    ...
    draw(ctx) {
        const sprite = SpriteManager.get('hyrule');
        if (sprite && SpriteManager.loaded) {
            ...
            // Assuming sprite sheet is loaded, we use its dimensions
            ...
            // Fallback
            ctx.fillStyle = '#27ae60'; // Green Tunic
